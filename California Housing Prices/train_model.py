...
class TrainModel():
	...
	def train_model(self):
		...
		self.mse_score = cross_val_score(self.model,self.train_prepared,self.train_label, scoring="neg_mean_squared_error",cv=10)
	...
	def save_model(self):
		...
		if self.modelname != "":
			if not os.path.exists("trainmodels") : os.makedirs("trainmodels")
...
class GridSearchModel():
	def __init__(self,modelname, model, param_grid, train_prepared,train_label):
		...
		if modelname =="" : modelname = "gridsearch_model"
		self.modelname = modelname
		self.modelfilename = "trainmodels\\" + self.modelname + ".pkl"
	...
	def train_model(self, showdetail = False):
		...
		self.rmse_score = np.sqrt(-self.mse_score)
		self.save_model()
		myprint("%20s : Mean RMSE Score = %f  : Best Params = %s " %(self.modelname, self.rmse_score.mean(),str(self.best_params)))
